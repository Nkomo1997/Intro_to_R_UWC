...
plot(TukeyHSD(chicks.aov1))#need to interpret tablel

summary(aov(weight ~ Diet, data = filter(chicks, Time %in% c(0))))

TukeyHSD(aov(weight ~ Diet, data = filter(chicks, Time %in% c(0))))

# The script is once again neat and clean 
# Lack of comments
# The same issues applies as in day 1
# Copying code from the book does not increase you skills or your coding knowledge
# Try new things
# Try differet things